...
export default function CentroNotificaciones({ clientes, creditos, showToast }) {
  ...
  const getMensaje = () => {
    ...
    if (plantilla === 0) return '⚠️ *Recordatorio*\n\nEstimado ' + nombre + ', su credito por ' + monto + ' vence en ' + dias + ' dias, el ' + fecha + '. Por favor realice su pago a tiempo.';
    if (plantilla === 1) return '🚨 *Credito vencido*\n\nEstimado ' + nombre + ', su credito por ' + monto + ' vencio el ' + fecha + '. Por favor realice su pago lo mas pronto posible.';
        ...
        <div>
          ...
          <div style={{ background: '#e5ddd5', borderRadius: '8px', padding: '10px', marginBottom: '10px', minHeight: '80px' }}>
            <div style={{ background: 'white', borderRadius: '0 8px 8px 8px', padding: '8px 10px', fontSize: '12px', color: '#111', lineHeight: 1.5 }}>
              {cliente && deuda ? getMensaje().split('\n').map((line, i) => <span key={i}>{line}<br /></span>) : <span style={{ color: '#999', fontSize: '11px' }}>Selecciona un cliente y una deuda</span>}
            </div>
          </div>
