...
export async function POST(req) {
  ...
  const { cliente_id, empresa_id, mes, comision, monto_servicio } = body;
  if (!cliente_id || !empresa_id || !mes) return Response.json({ error: 'Faltan datos' }, { status: 400 });
  const { data, error } = await db().from('recargas').insert({ cliente_id, empresa_id, mes, comision: comision || 0, monto_servicio: monto_servicio || 0 }).select().single();
